...
class MyDataCatalog(DataCatalog):
    def load_all_data(self) -> pd.DataFrame:
            merged_df = CSVDataSet(filepath=parent_directory+"/data/02_intermediate/processed_merged_df.csv")
    ...
    def load_batsman_data(self) -> pd.DataFrame:
            ...
            batsman_df = CSVDataSet(filepath=parent_directory+"/data/02_intermediate/batsman_df_1.csv", load_args=None, save_args={'index': False})
    ...
    def load_bowler_data(self) -> pd.DataFrame:
            # COMMENT: Data loading code written by ChatGPT was incorrect
            bowler_df = CSVDataSet(filepath=parent_directory+"/data/02_intermediate/bowler_df_1.csv", load_args=None, save_args={'index': False})
    ...
    def load_bowler_forecasting_data(self) -> pd.DataFrame:
                # COMMENT: Data loading code written by ChatGPT was incorrect
            predicted_bowl = CSVDataSet(filepath=parent_directory+"/data/07_model_output/prediction_bowl.csv", load_args=None, save_args={'index': False})
    ...
    def load_batsman_forecasting_data(self) -> pd.DataFrame:
                # COMMENT: Data loading code written by ChatGPT was incorrect
            predicted_bat = CSVDataSet(filepath=parent_directory+"/data/07_model_output/prediction_bat.csv", load_args=None, save_args={'index': False})
